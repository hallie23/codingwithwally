...
def op_read(args):
    global registers, reads
    try:
        registers[args[0]] = int(reads[0])
        reads = reads[1:]
        print("read", registers[args[0]])
        if not validInteger(registers[args[0]]):
            print("\nIllegal input: number must be in [-32768,32767]\n")
    except:
        print("\nNot enough inputs given for this code\n")
    # try:
    #     sys.stdout.flush()
    #     sys.stderr.flush()
    # except:
...
def op_write(args):
    global registers, writes
    writes += [registers[args[0]]]       #EDIT
